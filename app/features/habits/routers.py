...
from app.features.habits.services import create_habit, update_habit, delete_habit, toggle_habit, toggle_habit_log
...
async def list_habits_route(request: Request, db: AsyncSession = Depends(get_db), current_user:
User = Depends(get_current_active_user)):
    if current_user.role == UserRole.admin:
        stmt = select(Habit)
    else:
        stmt = select(Habit).where(Habit.owner_id == current_user.id)
    ...
    for h in habits:
        ...
        logs = (await db.execute(
            select(HabitLog).where(HabitLog.habit_id == h.id, HabitLog.timestamp.in_(header)))).scalars().all()
...
async def add_habit_route(
    ...
    await create_habit(db, owner_id=current_user.id,
                       data=HabitCreate(name=name, description=description, frequency=frequency))
